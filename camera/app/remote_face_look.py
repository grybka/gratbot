...
class GratbotClient:
    ...
    def send_message(address,command,arguments):
        message={"address": address,"commmand": command,"arguments": arguments}
        sock.sendall(yaml.safe_dump(message)+"\n")
        data=self.sock.recv(1024)
        while len(data)>0 & data[-1]!="\n":
            data+=sock.recv(1024)
...
def move_camera_to(pitch,yaw):
    response=gratbot.send_message(["camera_pitch_servo","position"],"SET",pitch)
    if "error" in response:
        raise Exception("Error sending command: "+response["error"])
    response=gratbot.send_message(["camera_yaw_servo","position"],"SET",yaw)
    if "error" in response:
        raise Exception("Error sending command: "+response["error"])
